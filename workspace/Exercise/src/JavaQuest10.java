...
public class JavaQuest10 {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < my_array.length - 1; i++) {
      ...
      if (indexPosition == my_array.length - 1) { // go to print the original array straight
        break;
      } else {
        newArr[i] = my_array[i];
        if (i == indexPosition) {
          ...
          for (int j = i; j < my_array.length - 1; j++) {
            backup1 = my_array[j + 1];
            newArr[j + 1] = backup;
            backup = backup1;
          }
          break;
    ...
    // my_array = newArr;
    System.out.println("New Array: " + Arrays.toString(indexPosition == my_array.length - 1 ? my_array : newArr));
